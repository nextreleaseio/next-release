...
const PricingCard = ({ classNames, isMonthly, plan }) => {
    classNames = cx(classNames, 'px-10 py-6 sm:w-full md:w-3/12 flex flex-col');
    if (plan.feature) {
        classNames += ' bg-white shadow';
    }

    let buttonClass = 'btn block text-center hover:-translate-1';

    if (plan.feature) {
        buttonClass += ' btn-orange';
    } else {
        buttonClass += ' btn-purple hover:bg-orange-600';
    }
                ...
                <div className="mt-4">
                    <span className="text-3xl font-extrabold">
                        {isMonthly
                            ...
                            : plan.prices.annual.price}
                    </span>
                    <span className="text-grey-600">
                        {isMonthly
                            ? plan.prices.month.term
                            : plan.prices.annual.term}
                    </span>
                </div>
                <div className="mt-4">
                    <a href="https://my.nextrelease.io" className={buttonClass}>
                        Sign Up
                    </a>
                </div>
                <div className="mt-4">
