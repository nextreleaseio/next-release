...
const authors = {
    donald: {
        github: 'donaldwasserman',
        twitter: 'donaldwasserman',
        picture: donaldPic,
        fullName: 'Donald Wasserman',
        email: 'donald@nextrelease.io'
    },
    devon: {
        github: 'bleib1dj',
        twitter: 'bleib1dj',
...
const AuthorDetails = ({ name }) => {
    const { github, twitter, picture, fullName, email } = authors[name];
    const githubUrl = `https://www.github.com/${github}`;
    const twitterUrl = `https://twitter.com/${twitter}`;
    const mailTo = `mailto:${email}?`;
    return (
        <div className="flex sm:flex-row items-start md:flex-col">
            <img
                src={picture}
